...
Release:        1%{?dist}
Epoch:          2
Summary:        HypeMaterialShell - Material 3 inspired shell (git nightly)
...
and fixes. Includes pre-built hype CLI binary and QML shell files.

%prep
%setup -q -n hype-git-source
...
# Build hype-cli from source using vendored dependencies
...
%posttrans
if [ -d "%{_sysconfdir}/xdg/quickshell/hype" ]; then
    rmdir "%{_sysconfdir}/xdg/quickshell/hype" 2>/dev/null || true
