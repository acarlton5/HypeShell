...
func appendLogEnv(env []string) []string {
	if v := os.Getenv("HYPE_LOG_LEVEL"); v != "" {
		env = append(env, "HYPE_LOG_LEVEL="+v)
	}
	if v := os.Getenv("HYPE_LOG_FILE"); v != "" {
		env = append(env, "HYPE_LOG_FILE="+v)
	}
	return env
}
...
func runShellInteractive(session bool) {
	...
	cmd.Env = append(os.Environ(), "HYPE_SOCKET="+socketPath, "HYPE_SOCKET="+socketPath)
	...
	if isSessionManaged && hasSystemdRun() {
		cmd.Env = append(cmd.Env, "HYPE_DEFAULT_LAUNCH_PREFIX=systemd-run --user --scope")
	}

	homeDir, err := os.UserHomeDir()
	if err == nil && os.Getenv("HYPE_DISABLE_HOT_RELOAD") == "" {
		if !strings.HasPrefix(configPath, homeDir) {
			cmd.Env = append(cmd.Env, "HYPE_DISABLE_HOT_RELOAD=1")
		}
	}
