...
func init() {
	...
	runCmd.Flags().String("log-level", "", "Log level: debug, info, warn, error, fatal (overrides HYPE_LOG_LEVEL)")
	runCmd.Flags().String("log-file", "", "Append logs to this file in addition to stderr (overrides HYPE_LOG_FILE)")
	...
	setupCmd.AddCommand(setupBindsCmd, setupLayoutCmd, setupColorsCmd, setupOutputsCmd, setupCursorCmd, setupWindowrulesCmd)
