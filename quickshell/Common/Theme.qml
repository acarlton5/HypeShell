...
Singleton {
    ...
    function applyHyprlandColorsFromCurrentTheme() {
        ...
        const active = _rgbHex(themeData.primary || primary, "#d0bcff");
        const inactive = _rgbHex(themeData.outline || outline, "#948f99");
        const errorColor = _rgbHex(themeData.error || error, "#f2b8b5");
        const content = "# Auto-generated by HypeShell - do not edit manually\n"
                + "# Remove source = ./hype/colors.conf from your config to override.\n\n"
                + "$primary  = rgb(" + active + ")\n"
                + "$outline  = rgb(" + inactive + ")\n"
                + "$error    = rgb(" + errorColor + ")\n\n"
                + "general {\n"
                ...
                + "  }\n"
                + "}\n";
        const colorsPath = configDir + "/hypr/hype/colors.conf";
        const script = "mkdir -p " + shellQuote(configDir + "/hypr/hype")
                + " && printf '%s' " + shellQuote(content)
                + " > " + shellQuote(colorsPath)
                + " && { hyprctl reload >/dev/null 2>&1 || true; }";

        Proc.runCommand("hypr-theme-colors", ["sh", "-c", script], (output, exitCode) => {
            if (exitCode !== 0)
                log.warn("Failed to write Hyprland theme colors:", output);
        });
    }

    function _rgbHex(colorValue, fallback) {
        if (colorValue && colorValue.r !== undefined && colorValue.g !== undefined && colorValue.b !== undefined)
            return _hexByte(colorValue.r) + _hexByte(colorValue.g) + _hexByte(colorValue.b);

        let value = String(colorValue || fallback || "").trim();
        if (value.startsWith("#"))
        ...
        if (value.length >= 6)
            return value.substring(0, 6).toLowerCase();
        if (fallback && value !== String(fallback).replace("#", ""))
            return _rgbHex(fallback, "");
        return "d0bcff";
    }

    function _hexByte(unitValue) {
        const n = Math.max(0, Math.min(255, Math.round(Number(unitValue) * 255)));
