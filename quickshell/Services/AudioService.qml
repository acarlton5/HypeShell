...
`;
    ...
    function generateWireplumberConfig() {
        let config = "# Generated by HypeShell - Audio Device Aliases\n";
    ...
    function playLoginSoundIfApplicable() {
        if (SettingsData.soundsEnabled && SettingsData.soundLogin && !notificationsAudioMuted) {
            ...
            // to prevent it from playing on every HypeShell restart during the session
            ...
            const loginFile = `${runtimeDir}/hypeshell.login-${sessionId}`;
