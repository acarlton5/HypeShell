...
Item {
    id: root
    readonly property var log: Log.scoped("HYPEShell")
    ...
    Connections {
        ...
        function onForceHypeBarLayoutRefresh() {
            root.recreateBarSurfaces();
        }
    }
    ...
    Repeater {
        ...
        delegate: Loader {
            ...
            sourceComponent: HypeBar {
                ...
                hyprlandOverviewLoader: hypeBarRepeater.hyprlandOverviewLoaderRef
    ...
    Loader {
        id: hypeDashPopoutLoader

        active: false
        asynchronous: false

        Component.onCompleted: {
            PopoutService.hypeDashPopoutLoader = hypeDashPopoutLoader;
        }

        onLoaded: {
            if (item) {
                PopoutService.hypeDashPopout = item;
                PopoutService._onHypeDashPopoutLoaded();
            }
        }
    ...
    LazyLoader {
        id: hypeLauncherV2ModalLoader

        active: false

        Component.onCompleted: {
            PopoutService.hypeLauncherV2ModalLoader = hypeLauncherV2ModalLoader;
        }

        HypeLauncherV2Modal {
            id: hypeLauncherV2Modal

            Component.onCompleted: {
                PopoutService.hypeLauncherV2Modal = hypeLauncherV2Modal;
                PopoutService._onHypeLauncherV2ModalLoaded();
            }
        }
    }
    ...
    Connections {
        ...
        function onOpenUrlRequested(url) {
            if (url.startsWith("hype://theme/install/") || url.startsWith("hype://theme/install/")) {
                var themeId = url.replace(/^hype:\/\/theme\/install\//, "").replace(/^hype:\/\/theme\/install\//, "").split(/[?#]/)[0];
                if (themeId) {
            ...
            if (url.startsWith("hype://plugin/install/") || url.startsWith("hype://plugin/install/")) {
                var pluginId = url.replace(/^hype:\/\/plugin\/install\//, "").replace(/^hype:\/\/plugin\/install\//, "").split(/[?#]/)[0];
                if (pluginId) {
    ...
    HYPEShellIPC {
        ...
        hypeDashPopoutLoader: hypeDashPopoutLoader
        notepadSlideoutVariants: notepadSlideoutVariants
        hyprKeybindsModalLoader: hyprKeybindsModalLoader
        hypeBarRepeater: hypeBarRepeater
