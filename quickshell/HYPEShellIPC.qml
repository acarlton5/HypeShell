...
Item {
    id: root
    readonly property var log: Log.scoped("HYPEShellIPC")
    ...
    required property var hypeDashPopoutLoader
    required property var notepadSlideoutVariants
    required property var hyprKeybindsModalLoader
    required property var hypeBarRepeater
    ...
    function getPreferredBar(refPropertyName) {
        ...
        const loaders = Array.from({
            length: root.hypeBarRepeater.count
        }, (_, i) => root.hypeBarRepeater.itemAt(i));
    ...
    IpcHandler {
        ...
        function open(tab: string): string {
            ...
            const dash = root.hypeDashPopoutLoader.item;
        ...
        function close(): string {
            if (root.hypeDashPopoutLoader.item) {
                root.hypeDashPopoutLoader.item.dashVisible = false;
        ...
        function toggle(tab: string): string {
            if (root.hypeDashPopoutLoader.item?.dashVisible) {
                root.hypeDashPopoutLoader.item.dashVisible = false;
    ...
    IpcHandler {
        ...
        function openWith(mode: string): string {
            ...
            PopoutService.openHypeLauncherV2WithMode(mode);
